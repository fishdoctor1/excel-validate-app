...
const TEMPLATE_XLSX = path.join(TEMPLATE_DIR, "accounting_validation_template.xlsx");
const TEMPLATE_CSV  = path.join(TEMPLATE_DIR, "accounting_validation_template.csv");

const SQL_VALIDATION_SUFFIX = `
SELECT
  e.document_no,
  e.account_code,
  e.plan_type,
  ...
  CASE
    ...
    ELSE array_remove(ARRAY[
      ...
      CASE WHEN NULLIF(NULL::text,'') IS DISTINCT FROM NULLIF(e.failure_type::text,'') THEN 'failure_type' END,
      CASE WHEN NULLIF(NULL::text,'') IS DISTINCT FROM NULLIF(e.failure_reason::text,'') THEN 'failure_reason' END,
  ...
  END) AS is_valid
FROM excel e
LEFT JOIN accounting.accounting_20260112_success_events db
...
function ensureTemplateFiles() {
  ...
  ws.columns = ALL_HEADERS_TEMPLATE.map(h => ({ header: h, key: h, width: Math.min(35, Math.max(12, h.length + 4)) }));
...
function readXlsxToMatrix(buffer) {
  ...
  const matrix = XLSX.utils.sheet_to_json(ws, { header: 1, raw: true, defval: undefined });
  return matrix;
}
...
function extractFromMatrix(matrix, sourceKind) {
  ...
  for (let r = 1; r < matrix.length; r++) {
    ...
    const allEmpty = cToAi.every(v => v === null || v === undefined || (typeof v === "number" && Number.isNaN(v)));
    ...
    const obj = {
      excel_row,
      table_selector: (sourceKind === "csv" ? csvCellToValue(table_selector_raw) : (isEmptyCell(table_selector_raw) ? null : table_selector_raw))
    };
...
function generateSql(rows, confirmText) {
//   if (confirmText !== "Extract") {
//     return {
//       ok: false,
//       error: 'ต้องยืนยันคำว่า "Extract" ก่อน จึงจะ generate SQL ได้'
//     };
//   }
  ...
  function literalWithCast(colName, value) {
    ...
    // keep value as-is as much as possible; only stringify for SQL literal
    if (cast === "::int") {
      // avoid normalizing strings; if number, keep number
      if (typeof value === "number") return `${Math.trunc(value)}${cast}`;
      return `'${sqlEscapeText(value)}'${cast}`;
    }

    if (cast === "::numeric(18,2)") {
      if (typeof value === "number") return `'${String(value)}'${cast}`;
      return `'${sqlEscapeText(value)}'${cast}`;
    }
...
app.post("/api/extract", upload.single("file"), (req, res) => {
  ...
  try {
    ...
    if (ext === ".xlsx" || ext === ".xls") {
      ...
      return res.status(400).json({ ok: false, error: "รองรับเฉพาะ .xlsx/.xls/.csv" });
    }
